...
7. **Spec review loop** - dispatch spec reviewer subagent; fix issues and re-dispatch until approved (max 5 iterations, then surface to human). See `references/spec-document-reviewer-prompt.md` for the prompt template.
...
1. Dispatch a general-purpose subagent to review the spec (see `references/spec-document-reviewer-prompt.md` for the prompt template)
...
**Note:** The visual companion requires the upstream `scripts/` directory from [obra/superpowers](https://github.com/obra/superpowers/tree/main/skills/brainstorming/scripts). See `references/visual-companion.md` for the full guide.
...
If they agree to the companion, read `references/visual-companion.md` for the detailed guide before proceeding.
